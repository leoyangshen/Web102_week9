...
const useDebounce = (value, delay) => {
    ...
    useEffect(() => {
        // Set up a timer to update the debounced value after a delay
...
const App = () => {
    ...
    const [localSearchTerm, setLocalSearchTerm] = useState(''); // New local state for input value
    const [searchQuery, setSearchQuery] = useState(''); // Debounced state for filtering
    const [commentCounts, setCommentCounts] = useState({});

    // Use the custom debounce hook for the search query
    ...
    useEffect(() => {
        ...
        let filteredAndSortedPosts = [...allPosts];
        
        // Apply search filtering using the debounced value
        if (debouncedSearchQuery) {
            filteredAndSortedPosts = filteredAndSortedPosts.filter(post =>
        ...
        // Apply sorting
        if (sorting === 'newest') {
            filteredAndSortedPosts.sort((a, b) => b.createdAt - a.createdAt);
        } else if (sorting === 'popular') {
            filteredAndSortedPosts.sort((a, b) => (b.upvotes?.length || 0) - (a.upvotes?.length || 0));
        }

        // Update the state for displayed posts
        setPosts(filteredAndSortedPosts);

    }, [allPosts, debouncedSearchQuery, sorting]);
            ...
            <div id="posts-container">
                {posts.length === 0 ? (
                    <p className="empty-state">
                        {localSearchTerm ? 'No posts match your search.' : (userId ? 'No posts yet. Be the first to create one!' : 'Loading posts...')}
                    </p>
                ) : (
                    posts.map(post => (
